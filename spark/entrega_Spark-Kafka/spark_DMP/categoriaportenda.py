...
schema = StructType(
	...
	StructField('productos', StringType(), True),
        StructField('prezos', StringType(), True)
    ]
)
...
df2.createOrReplaceTempView("categorias")
df_sql = spark.sql("SELECT categoria, SUM(prezos) as beneficio FROM categorias GROUP BY categoria")

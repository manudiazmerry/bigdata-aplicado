...
appName = "Script Visor de vendas por Tenda"
master = "local"
...
df2.createOrReplaceTempView('ventas1')
#Creamos a consulta que imos empregar
consulta = spark.sql("SELECT Tenda, count(produto) as Num_Productos,sum(prezo) as Beneficios from ventas1 GROUP BY Tenda")
